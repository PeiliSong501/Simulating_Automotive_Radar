...
def process_frame(frame):
    radar_arg_file, radar_eagle_file, lidar_file, imu_file, cam_left_file, cam_right_file = get_snail_dir(df, frame,
                                                                                                          'data4')
    T_radar = get_trans('Ars548','Zed2LeftCam')
    if os.path.exists(f'D:/snail_radar/20231208/data4/pmap_image/{frame}.jpg'):
        return
    ...
    r_in = np.load(f'D:/snail_radar/20231208/data4/r_in/{frame}.npy')
    ...
    np.save(f'D:/snail_radar/20231208/data4/radar_2d_map_pro/{frame}.npy',
            radar_2d_map_pro)
    ...
    cv2.imwrite(f'D:/snail_radar/20231208/data4/pmap_image/{frame}.jpg',
                gray_image_pro)
...
if __name__ == '__main__':
    ...
    #     radar_pcd = read_pcd_to_numpy(radar_arg_file)
    ...
    #
    #
    #     coord = lidar_fov_to_image(T_radar, K, 120, 40, image.shape)
    #     print(coord)
    #
    #     coord = lidar_fov_to_image(T_radar, K, 180, 31, image.shape)
    #     print(coord)
    #
    #     coord = lidar_fov_to_image_asy(T_radar, K, 120, -25, 25, image.shape)
    #     print(coord)
    #
    ...
    #     print('frame', frame)
    #     radar_arg_file, radar_eagle_file, lidar_file, imu_file, cam_left_file, cam_right_file = get_snail_dir(df, frame,'data4')
    #     if not (os.path.exists(radar_arg_file) and os.path.exists(lidar_file) and os.path.exists(
    #             cam_left_file)):
...
#         np.save(f'D:/snail_radar/20231208/data4/l_in_2d/{frame}.npy',l_in_2d)
#         np.save(f'D:/snail_radar/20231208/data4/l_in/{frame}.npy',l_in)
#         r_in, r_in_2d = utils1.radar_in_image(coor, radar_pcd, T_radar,K)
#         np.save(f'D:/snail_radar/20231208/data4/r_in_2d/{frame}.npy',r_in_2d)
#         np.save(f'D:/snail_radar/20231208/data4/r_in/{frame}.npy',r_in)
...
#--------------------------------------------make dataset---------------------------------------------------------------
    ...
    #     T_l2r = get_trans('Xt32', 'Ars548')
    #
    #     radar_pcd = np.load(f'D:/snail_radar/20231208/data4/r_in/{frame}.npy')
    ...
    # new_df.to_csv('D:/snail_radar/20231208/data4/prob_dataset.csv', index=False)
    ...
    # v_max = 0
    # for frame in range(0, len(df)):
    #     r_in_2d = np.load(f'D:/snail_radar/20231208/data4/l_in_2d/{frame}.npy')
    ...
    #
    # # 读取数据集
    # df = pd.read_csv('D:/snail_radar/20231208/data4/prob_dataset.csv').iloc[500:4648,:]
    ...
    #
    # # 保存划分后的数据集（可选）
    # train_df.to_csv('D:/snail_radar/20231208/data4/train_dataset.csv',
    #                 index=False)
    # test_df.to_csv('D:/snail_radar/20231208/data4/test_dataset.csv', index=False)
    #
    # #检查PointNum列的最小值和最大值
    # print(np.min(train_df['PointNum'].values), np.max(train_df['PointNum'].values))

    # -------------------------------------------------RCS regression--------------------------------------------------------
    # train_df = pd.read_csv("D:/snail_radar/20231208/data4/train_dataset.csv").values
    # test_df = pd.read_csv("D:/snail_radar/20231208/data4/test_dataset.csv").values
    ...
    #     l_in = np.load(
    #         f'D:/snail_radar/20231208/data4/l_in/{frame}.npy')
    #     #l_in_radar_coor = utils.trans_point_coor(l_in, T_lidar2radar)
    #     l_in_radar_coor = l_in
    #     # np.save(f'D:/snail_radar/20231208/data4/l_in_radar_coor/{frame}.npy',
    #     #         l_in_radar_coor)
    ...
    #
    #         np.save(f'D:/snail_radar/20231208/data4/local_points/{frame}_{j}.npy',
    #                 local_lidar_radarcoor)
    #         dst = f'D:/snail_radar/20231208/data4/range_image/{frame}_{j}.jpg'
    ...
    #                             dst)
    #         #idx += 1
    # new_df_train.to_csv('D:/snail_radar/20231208/data4/RCS_dataset_train.csv')
    ...
    #
    #     local_lidar_radarcoor = l_in_radar_coor[local_lidar_index]
    #     # np.save(f'D:/snail_radar/20231208/data4/local_points/{int(frame)}_{int(index)}.npy',
    #     #         local_lidar_radarcoor)
    #     # local_lidar_radarcoor = np.load(f'D:/Astyx dataset/dataset_astyx_hires2019/local_points/{int(frame)}_{int(index)}.npy')
    #     dst = f'D:/snail_radar/20231208/data4/ablation/range_image/{int(frame)}_{int(index)}.jpg'
    #     # virtual_point = get_new_point(x, y, z)
    #     fov_down = -25
    #     fov_up = 15
    #     # for k in range(len(local_lidar_radarcoor)):
    ...
    for r in [0.5,2,3,4]:
        def process_data(row):
            ...
            l_in_radar_coor = np.load(f'D:/snail_radar/20231208/data4/l_in/{int(frame)}.npy')
            kdtree = KDTree(l_in_radar_coor)
            if not os.path.exists(f'D:/snail_radar/20231208/data4/ablation/newest/range_image_{r}'):
                os.mkdir(f'D:/snail_radar/20231208/data4/ablation/newest/range_image_{r}')
            ...
            dst = f'D:/snail_radar/20231208/data4/ablation/newest/range_image_{r}/{int(frame)}_{int(index)}.jpg'
            fov_down = -25
            fov_up = 15
        ...
        df = pd.read_csv('D:/snail_radar/20231208/data4/RCS_dataset_train.csv').values[:, 1:]
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            executor.map(process_data, df)