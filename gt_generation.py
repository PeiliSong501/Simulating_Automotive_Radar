...
This script contains utility functions and data-generation routines used to
build radar-related ground-truth / target representations for the simulation
pipeline. The file includes:
1. Radar/LiDAR projection and field-of-view filtering utilities.
2. Probabilistic radar target map generation in the image plane.
3. Patch / voxel / range-image generation helpers for dataset preparation.
4. Experimental and ablation data-generation code kept for reproducibility.

For open-source reproducibility, editable dataset paths are grouped at the top
of the file so users only need to modify a few variables.
"""
...
# Modify the paths below to match your local dataset layout.
# =============================================================================
DATA_ROOT = "/workspace/data/VoD_dataset/view_of_delft_PUBLIC"
RADAR_TRAIN_DIR = os.path.join(DATA_ROOT, "radar", "training")
PMAP_DIR = os.path.join(RADAR_TRAIN_DIR, "PMapDataset")
IMAGE_2_DIR = os.path.join(RADAR_TRAIN_DIR, "image_2")
...
def sample_voxels(center_list, num_frames=9931, frame_interval=15, voxels_per_frame=200,
                  save_dir=f"{RADAR_TRAIN_DIR}/voxel_dataset/sample_voxel/"):
...
def process_frame(frame):
    if os.path.exists(f'{PMAP_DIR}/pmap_image/{frame}.jpg'):
        return
    ...
    np.save(f'{PMAP_DIR}/r_in/{frame}.npy',r_in)
    np.save(f'{PMAP_DIR}/r_in_2d/{frame}.npy', r_in_2d)
    ...
    np.save(f'{PMAP_DIR}/radar_2d_map_pro/{frame}.npy',
            radar_2d_map_pro)
    ...
    cv2.imwrite(f'{PMAP_DIR}/pmap_image/{frame}.jpg',
                gray_image_pro)
...
#------------------------------------------generate radar probabilistic image------------------------------------------
    ...
    #     r_in, r_in_2d = radar_in_image(coor, radar_pcd,T_radar,K)
    #     #np.save(f'{PMAP_DIR}/radar_in/{frame}.npy',r_in)
    #     rcs_list = r_in[:,3]
    #     imgage = cv2.imread(cam_file)
    #
    #     #radar_2d_map = np.zeros((896,1935))
    #
    #     if os.path.exists(f'{PMAP_DIR}/radar_2d_map_pro/{frame}.npy'):
    #         radar_2d_map_pro = np.load(f'{PMAP_DIR}/radar_2d_map_pro/{frame}.npy')
    #         radar_2d_map_pro = radar_2d_map_pro[252:,0:1935]
    #
    #     # standard_variance = [100,100]
    #     # radar_2d_map_pro = update_radar_map_with_gaussian(radar_2d_map, r_in, r_in_2d, standard_variance)
    #     #np.save(f'{PMAP_DIR}/radar_2d_map_pro/{frame}.npy', radar_2d_map_pro)
    #
    #         # normalized_array = cv2.normalize(radar_2d_map_pro, None, 0, 255, cv2.NORM_MINMAX)
    #         # gray_image_pro = normalized_array.astype(np.uint8)
    #
    #         # cv2.imwrite(f'{PMAP_DIR}/pmap_image/{frame}.jpg', gray_image_pro)
    #         gray_image_pro = cv2.imread(f'{PMAP_DIR}/pmap_image/{frame}.jpg')
    #         gray_image_pro = gray_image_pro[252:,0:1935]
    #         cv2.imwrite(f'{PMAP_DIR}/pmap_image/{frame}.jpg', gray_image_pro)
    #
    #
    #
    # # radar_2d_map_pro_image = cv2.imread(f'{PMAP_DIR}/pmap.jpg', cv2.IMREAD_GRAYSCALE)
...
#--------------------------------------pointinside& larger input image--------------------------------------------------
    ...
    # ego_velo = np.load(f'{RADAR_TRAIN_DIR}/radar_velo.npy')
    # for frame in range(0,9931,1):
    #     radar_file, lidar_file, cam_file, calib_file, lidar_calib_file, txt_base_dir = utils.get_vod_dir(frame)
    #     if not (os.path.exists(radar_file) and os.path.exists(calib_file)):
    #         print('file not exist!')
    #         continue
    #     if os.path.exists(f'{PMAP_DIR}/pmap_image/{frame}.jpg'):
    #         r_in = np.load(f'{PMAP_DIR}/r_in/{frame}.npy')
    #         pointnum = len(r_in)
    ...
    # #     new_data = pd.DataFrame({'Frame': [frame], 'PointNum': [pointnum], 'Velocity': [v]})
    # #     new_df = pd.concat([new_df, new_data], ignore_index=True)
    # new_df.to_csv(f'{PMAP_DIR}/prob_dataset.csv', index=False)
    # print(new_df)

    # df = pd.read_csv(f'{PMAP_DIR}/prob_dataset.csv')
    # print(df.values.shape)

#-------------------------------------------generate lidar target image-------------------------------------------------
    # lidar_base = f'{PMAP_DIR}/lidar_inter2d_pic/'
    ...
    #     coords = np.load(f'{PMAP_DIR}/cropped_image_coor/{frame}.npy')
    ...
    #         cv2.imwrite(f'{PMAP_DIR}/target_lidar/{frame}_{i}.jpg',local_lidar_depth_tmp)
...
#--------------------------------------------------generate LiDAR depth map---------------------------------------------
    ...
    #     np.save(f'{PMAP_DIR}/lidar_2d/{frame}.npy',l_in_2d)
    #     np.save(f'{PMAP_DIR}/lidar2d_corr_3dpoints/{frame}.npy',l_in)
    #
    #
    #
    # frames = range(0, 9931)
    ...
    #     np.save(f'{RADAR_TRAIN_DIR}/velodyne_pixel/{frame}.npy',filtered_points_2d)
    #     if frame % 10 == 0:
    ...
    #     np.save(f'{PMAP_DIR}/lidar_pixel/{str(frame)}.npy',filtered_points_2d)
    #     if frame % 10 == 0:
    #         print(f'frame {frame} saved.')
    ...
    #
    #
    #     np.save(f'{PMAP_DIR}/cropped_image_coor/{frame}.npy',coordinates)
    #     for i in range(len(crops)):
    #         print(f'index {i}')
    #         c = crops[i]
    #         cv2.imwrite(
    #             f'{PMAP_DIR}/cropped_images/{frame}_{i}.jpg',
    #             c)
    #         coor = coordinates[i]
    #         u1,v1 = coor[0]
    #         u2,v2 = coor[1]
    #         target_image = cv2.imread(f'{PMAP_DIR}/pmap_image/{frame}.jpg')
    #         crop_target = target_image[v1-319:v2-319, u1:u2]
    #         print('crop_target:',u1,v1-319,u2,v2-319)
    #         cv2.imwrite(f'{PMAP_DIR}/target_image/{frame}_{i}.jpg', crop_target)
    #         print('coor_1:',u1,v1,'coor_2:',u2,v2)
    #         new_data = pd.DataFrame({'Frame':[frame], 'Index':[i]})
    #         df = pd.concat([df, new_data], ignore_index=True)
    # df.to_csv(f'{PMAP_DIR}/prob_dataset.csv')
...
#-----------------------------------------------split dataset-----------------------------------------------------------
    ...
    #
    # # 保存训练集和测试集到 CSV 文件
    # train_data.to_csv(f'{PMAP_DIR}/train_dataset.csv', index=False)
    # test_data.to_csv(f'{PMAP_DIR}/test_dataset.csv', index=False)
...
#         cam_dir = f'{IMAGE_2_DIR}/'+str(int(frame)).zfill(5)+".jpg"
#         des_dir = f'{PMAP_DIR}/edge_image/'+str(int(frame))+".jpg"
...
#         print(edges.shape)
    ...
    # # 读取图像并转换为灰度图
    # image = cv2.imread(f'{IMAGE_2_DIR}/00240.jpg')
    # gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    #
    # # 边缘检测
    # edges = cv2.Canny(gray, 50, 150)
    ...
    # img = cv2.imread(f'{IMAGE_2_DIR}/09902.jpg', 0)
...
#--------------------------------------deciding covariance by 2D clustering---------------------------------------------
    ...
    #     # # np.save(f'{PMAP_DIR}/r_in_2d/{frame}.npy',r_in_2d_final)
    #     # # np.save(f'{PMAP_DIR}/r_in/{frame}.npy', r_in_final)
    #     annotations = utils.read_annotation(txt_base_dir+f'{str(frame).zfill(5)}.txt')
    #     if annotations is None:
    #         continue
    #     r_in_2d = np.load(f'{PMAP_DIR}/r_in_2d/{frame}.npy')
    #     r_in = np.load(f'{PMAP_DIR}/r_in/{frame}.npy')
...
# --------------------------------------------fix lidar depth-----------------------------------------------------------
#     df = pd.read_csv(f'{RADAR_TRAIN_DIR}/RNet_dataset.csv').values[:,1:]
#     for i in range(len(df)):
#         lidar_uv_depth = np.full((100, 100), -1)
#         frame = df[i, 0]
#         index = df[i, 1]
#         if df[i,3] > 0:
#             lidar_points = np.load(f'{RADAR_TRAIN_DIR}/cropped_lidar_radarcoor/{frame}_{index}.npy')
#             radar_file, lidar_file, cam_file, calib_file, lidar_calib_file, txt_base_dir = utils.get_vod_dir(frame)
#             #image = cv2.imread(f'{RADAR_TRAIN_DIR}/cropped_images/{frame}_{index}.jpg')
#             K = utils.get_intrinsic_matrix(calib_file)
#             if K is None:
#                 continue
#             T_radar = utils.get_radar2cam(calib_file)
#             T_lidar = utils.get_lidar2cam(lidar_calib_file)
#             coor = np.load(f'{RADAR_TRAIN_DIR}/cropped_image_coor/{frame}.npy')[index]
#             lidar_uv_depth, cnt_depth = lidar2uv_depth(lidar_points,T_radar,K,coor)
#             print(f'saved {frame}_{index}.npy, cnt_depth = {cnt_depth}')
#         np.save(f'{RADAR_TRAIN_DIR}/lidar_uv_depth/{frame}_{index}.npy',lidar_uv_depth)
...
#-----------------------------project lidar_uv_depth on cropped image-------------------------------------------
    ...
    # index = 4
    # image = cv2.imread(f'{RADAR_TRAIN_DIR}/cropped_images/{frame}_{index}.jpg')
    # pixel_matrix = np.load(f'{RADAR_TRAIN_DIR}/lidar_uv_depth/{frame}_{index}.npy')
    #
...
# --------------------------------------------------generate range image------------------------------------------------
    ...
    #     T_radar = utils.get_radar2cam(calib_file)
    #     T_lidar = utils.get_lidar2cam(lidar_calib_file)
    #     coords = np.load(f'{PMAP_DIR}/cropped_image_coor/{frame}_expanded.npy')
    #     for i in range(len(coords)):
    #         print(i)
    #         coor = coords[i]
    #         point1, point2 = coor
...
#-------------------------------------------make RCS regression dataset--------------------------------------
    ...
    # df = pd.read_csv(f'{PMAP_DIR}/RCS_dataset_train.csv').values[:,1:]
    # for i in range(len(df)):
    #
    #     frame, index, x, y, z, u, v, v_r, rcs = df[i]
    #     l_in_radar_coor = np.load(f'{PMAP_DIR}/l_in_radar_coor/{int(frame)}.npy')
    ...
    #
    #     dst = f'{PMAP_DIR}/ablation/range_image/{int(frame)}_{int(index)}.jpg'
    #     fov_down = -23
    #     fov_up = 7
    #
...
#-------------------------------make RCS regression dataset for optimal parameter fiding------------------------------
    for r in [0.5,1,2,3,4]:
        def process_data(row):
            ...
            local_lidar_index = kdtree.query_ball_point([x, y, z], r)
            local_lidar_radarcoor = l_in_radar_coor[local_lidar_index]
            if not os.path.exists(f'{PMAP_DIR}/ablation/range_image_{r}/'):
                os.mkdir(f'{PMAP_DIR}/ablation/range_image_{r}/')
            
            dst = f'{PMAP_DIR}/ablation/range_image_{r}/{int(frame)}_{int(index)}.jpg'
        ...
        df = pd.read_csv(f'{PMAP_DIR}/RCS_dataset_test.csv').values[:,
            1:]
        with concurrent.futures.ThreadPoolExecutor(max_workers=12) as executor:
            executor.map(process_data, df)
